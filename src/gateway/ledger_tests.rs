...
fn independent_units_reserve_atomically_and_settle_once() {
    ...
    ledger
        .set_account(u64::MAX, policy(BillingUnit::Cny))
        .unwrap();
    ledger
        .set_account(u64::MAX, policy(BillingUnit::Usd))
        .unwrap();
    ledger
        .reserve(reservation("first", BillingUnit::Cny, "0.7"))
        .unwrap();
    assert!(
        ledger
            .reserve(reservation("second", BillingUnit::Cny, "0.4"))
            .is_err()
    );
    ledger
        .reserve(reservation("dollars", BillingUnit::Usd, "0.4"))
        .unwrap();
    ...
    let usd = accounts
        .iter()
        .find(|a| a.policy.unit == BillingUnit::Usd)
        .unwrap();
...
fn pending_record(id: &str, committed: bool) -> AttemptInput {
    AttemptInput {
        ...
        usage: Some(serde_json::json!({"input_tokens": 17})),
    }
}
...
fn admission_validates_policy_bounds_zero_quota_and_authorization() {
    ...
    assert!(
        db.reserve(reservation("missing", BillingUnit::Cny, "0"))
            .is_err()
    );
    ...
    assert!(
        db.reserve(reservation("zero", BillingUnit::Cny, "0"))
            .is_err()
    );
    p.limit = None;
    db.set_account(u64::MAX, p.clone()).unwrap();
    ...
    db.set_account(u64::MAX, p.clone()).unwrap();
    assert!(
        db.reserve(reservation("forbidden", BillingUnit::Cny, "0"))
            .is_err()
    );
    ...
    assert!(
        db.reserve(reservation("concurrency", BillingUnit::Cny, "0"))
            .is_err()
    );
...
fn file_reopen_recovery_keeps_customer_exposure_and_exact_settlement() {
    let path = db_path();
    {
        ...
        db.reserve(reservation("crash", BillingUnit::Cny, "0.7"))
            .unwrap();
    }
    {
        let db = Ledger::open(&path).unwrap();
        ...
        assert_eq!(
            (a.in_flight, a.customer_pending, a.provider_only_pending),
            (0, 1, 0)
        );
        let mut p = policy(BillingUnit::Cny);
        p.max_pending = 1;
        ...
        assert!(
            db.reserve(reservation("blocked", BillingUnit::Cny, "0.1"))
                .is_err()
        );
    ...
    db.settle(s).unwrap();
    assert_eq!(
        db.accounts(u64::MAX).unwrap()[0].used,
        amount("0.123456789012345678")
    );
    assert_eq!(
        db.list_requests(u64::MAX, 10).unwrap()[0].attempts[0].state,
        ReservationState::Settled
    );
    let connection = rusqlite::Connection::open(&path).unwrap();
    let stored: (String, String) = connection
        .query_row("SELECT typeof(used),used FROM ledger_accounts", [], |r| {
            Ok((r.get(0)?, r.get(1)?))
        })
        .unwrap();
...
fn hidden_failure_releases_for_independent_currency_retry_and_preserves_cost() {
    ...
    db.reserve(reservation("hidden", BillingUnit::Cny, "0.7"))
        .unwrap();
    ...
    assert_eq!(
        (
            cny.used,
            cny.reserved,
            cny.customer_pending,
            cny.provider_only_pending
        ),
        (Amount::ZERO, Amount::ZERO, 0, 1)
    );
    ...
    assert_eq!(
        requests[0].attempts[0].record.as_ref().unwrap().cost.amount,
        Some(amount("0.9"))
    );
...
fn committed_partial_usage_stays_pending_then_resolves_once() {
    let db = Ledger::open_in_memory().unwrap();
    db.set_account(u64::MAX, policy(BillingUnit::Cny)).unwrap();
    db.reserve(reservation("partial", BillingUnit::Cny, "0.7"))
        .unwrap();
    ...
    assert_eq!(
        (
            ...
            a.reserved,
            a.customer_pending,
            a.provider_only_pending
        ),
        (amount("0.4"), Amount::ZERO, 0, 1)
    );
}
...
fn legacy_import_is_idempotent_and_preserves_existing_accounts() {
    let path = db_path();
    {
        ...
        let a = db
            .import_legacy(9, amount("0.0169543708291874"), Some(amount("2")))
            .unwrap();
    ...
    db.set_account(u64::MAX, p).unwrap();
    let a = db
        .import_legacy(u64::MAX, amount("7"), Some(amount("8")))
        .unwrap();
...
fn audited_adjustments_and_cycles_cannot_erase_unresolved_obligations() {
    let db = Ledger::open_in_memory().unwrap();
    db.set_account(u64::MAX, policy(BillingUnit::Cny)).unwrap();
    db.reserve(reservation("adjust", BillingUnit::Cny, "0.7"))
        .unwrap();
    let cycle = NewCycleInput {
        operation_id: "cycle1".into(),
        key_id: u64::MAX,
        unit: BillingUnit::Cny,
        reason: "new month".into(),
    };
    assert!(db.new_cycle(cycle.clone()).is_err());
    db.settle(settlement("adjust", "0.5")).unwrap();
    let adjustment = AdjustmentInput {
        adjustment_id: "refund".into(),
        key_id: u64::MAX,
        unit: BillingUnit::Cny,
        direction: AdjustmentDirection::Credit,
        amount: amount("0.2"),
        reason: "invoice correction".into(),
    };
    db.adjust(adjustment.clone()).unwrap();
    db.adjust(adjustment.clone()).unwrap();
    assert_eq!(db.accounts(u64::MAX).unwrap()[0].used, amount("0.3"));
    let mut invalid = adjustment.clone();
    invalid.adjustment_id = "bad".into();
    invalid.amount = amount("0.4");
    assert!(db.adjust(invalid).is_err());
    let mut invalid = adjustment;
    invalid.adjustment_id = "blank".into();
    invalid.reason = " ".into();
    assert!(db.adjust(invalid).is_err());
    ...
    assert_eq!(
        db.list_requests(u64::MAX, 10).unwrap()[0].attempts[0].cycle,
        1
    );
...
fn concurrent_connections_cannot_oversubscribe() {
    ...
    Ledger::open(&path)
        ...
        .set_account(u64::MAX, policy(BillingUnit::Cny))
        .unwrap();
    let db1 = Ledger::open(&path).unwrap();
    let db2 = Ledger::open(&path).unwrap();
    let barrier = std::sync::Arc::new(std::sync::Barrier::new(2));
    let b1 = barrier.clone();
    let one = std::thread::spawn(move || {
        b1.wait();
        db1.reserve(reservation("one", BillingUnit::Cny, "0.7"))
            .is_ok()
    });
    let two = std::thread::spawn(move || {
        barrier.wait();
        db2.reserve(reservation("two", BillingUnit::Cny, "0.7"))
            .is_ok()
    });
    assert_ne!(one.join().unwrap(), two.join().unwrap());
    assert_eq!(
        Ledger::open(&path).unwrap().accounts(u64::MAX).unwrap()[0].reserved,
        amount("0.7")
    );
}
...
fn reservation_overflow_rolls_back_atomically() {
    ...
    assert_eq!(
        db.accounts(u64::MAX).unwrap()[0].reserved,
        amount("170141183460469231731.687303715884105727")
    );
}
...
fn rejected_settlement_does_not_change_usage_or_commit_request() {
    let db = Ledger::open_in_memory().unwrap();
    db.set_account(u64::MAX, policy(BillingUnit::Cny)).unwrap();
    db.reserve(reservation("reject", BillingUnit::Cny, "0.7"))
        .unwrap();
    assert!(db.settle(settlement("reject", "0.8")).is_err());
    let mut wrong = settlement("reject", "0.4");
    wrong.attempt_cost.unit = BillingUnit::Cny;
    assert!(db.settle(wrong).is_err());
    let mut hidden = settlement("reject", "0.4");
    hidden.committed = false;
    hidden.outcome = AttemptOutcome::Failed;
    assert!(db.settle(hidden).is_err());
    let mut missing = settlement("reject", "0.4");
    missing.downstream_amount = None;
    ...
    let mut changed = settlement("reject", "0.6");
    changed.settlement_id = "different-id".into();
...
fn native_credit_soft_overage_is_exact_and_blocks_further_admission() {
    ...
    let mut p = policy(BillingUnit::KiroCredit);
    p.enforcement = BudgetEnforcement::Soft;
    p.limit = Some(amount("0.01"));
    db.set_account(u64::MAX, p).unwrap();
    let mut r = reservation("credit", BillingUnit::KiroCredit, "0.01");
    r.snapshot.upstream_kind = UpstreamKind::Kiro;
    r.snapshot.cost_prices = None;
    r.snapshot.sell_prices = None;
    assert!(db.reserve(r.clone()).is_err());
    r.bound_kind = BoundKind::Unknown;
    r.upper_bound = None;
    db.reserve(r.clone()).unwrap();
    let mut s = settlement("credit", "0.0169543708291874");
    s.attempt_cost.unit = BillingUnit::KiroCredit;
    s.attempt_cost.amount = Some(amount("0.0169543708291874"));
    db.settle(s).unwrap();
    let a = &db.accounts(u64::MAX).unwrap()[0];
    assert_eq!(a.used, amount("0.0169543708291874"));
    assert_eq!(a.available, Some(Amount::ZERO));
    r.request_id = "next-credit".into();
    r.attempt_id = "next-credit".into();
...
fn provider_only_pending_survives_cycle_and_account_update_preserves_usage() {
    let db = Ledger::open_in_memory().unwrap();
    db.set_account(u64::MAX, policy(BillingUnit::Cny)).unwrap();
    db.reserve(reservation("pending-cost", BillingUnit::Cny, "0.7"))
        .unwrap();
    let mut s = settlement("pending-cost", "0.5");
    s.attempt_cost = pending_record("pending-cost", true).cost;
    db.settle(s).unwrap();
    let mut p = policy(BillingUnit::Cny);
    p.limit = Some(amount("2"));
    assert_eq!(
        db.set_account(u64::MAX, p.clone()).unwrap().used,
        amount("0.5")
    );
    let a = db
        .new_cycle(NewCycleInput {
            operation_id: "new-cycle".into(),
            key_id: u64::MAX,
            unit: BillingUnit::Cny,
            reason: "monthly close".into(),
        })
        .unwrap();
    assert_eq!(
        (a.cycle, a.used, a.provider_only_pending),
        (2, Amount::ZERO, 1)
    );
    p.max_pending = 1;
    db.set_account(u64::MAX, p).unwrap();
    assert!(
        db.reserve(reservation("pending-gate", BillingUnit::Cny, "0.1"))
            .is_err()
    );
    let mut evidence = pending_record("pending-cost", true);
    evidence.outcome = AttemptOutcome::Succeeded;
    evidence.cost.evidence = EvidenceStatus::Confirmed;
    evidence.cost.amount = Some(amount("0.8"));
