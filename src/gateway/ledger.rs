...
impl Ledger {
    pub fn legacy_migration_complete(&self) -> anyhow::Result<bool> {
        self.transaction(migration_complete)
    }
    ...
    pub fn list_account_audit(
        &self,
        key_id: u64,
        limit: u32,
    ) -> anyhow::Result<Vec<AccountAuditView>> {
        self.transaction(|tx| {
    ...
    pub fn record_attempt(&self, input: AttemptInput) -> anyhow::Result<AttemptView> {
        ...
        self.transaction(|tx| {
            ...
            if event_matches(tx, &input.event_id, "attempt", &input)? {
                return Ok(view);
            }
            update_record(tx, &mut view, input.clone())?;
            if holds_customer(&view) {
    ...
    pub fn adjust(&self, input: AdjustmentInput) -> anyhow::Result<AccountView> {
        nonblank(&input.adjustment_id)?;
        nonblank(&input.reason)?;
        self.transaction(|tx| {
            let before = account(tx, input.key_id, input.unit)?;
            if event_matches(tx, &input.adjustment_id, "adjustment", &input)? {
                return Ok(before);
            }
            ...
            save_audit(
                tx,
                AccountAuditView {
                    operation_id: input.adjustment_id.clone(),
    ...
    pub fn new_cycle(&self, input: NewCycleInput) -> anyhow::Result<AccountView> {
        nonblank(&input.operation_id)?;
        nonblank(&input.reason)?;
        self.transaction(|tx| {
            let before = account(tx, input.key_id, input.unit)?;
            if event_matches(tx, &input.operation_id, "new_cycle", &input)? {
                return Ok(before);
            }
            ensure!(
                before.in_flight == 0 && before.customer_pending == 0,
                "cannot change cycle with customer obligations"
            );
            let next = before.cycle.checked_add(1).context("cycle overflow")?;
            tx.execute(
                "INSERT INTO ledger_cycles(key_id,unit,cycle,closing_account) VALUES(?1,?2,?3,?4)",
                params![
            ...
            tx.execute(
                "UPDATE ledger_accounts SET cycle=?1,used='0' WHERE key_id=?2 AND unit=?3",
    ...
    pub fn max_recorded_key_id(&self) -> anyhow::Result<Option<u64>> {
        self.transaction(|tx| {
            let rows = tx
                .prepare("SELECT DISTINCT key_id FROM ledger_accounts")?
    ...
    pub fn open(path: impl AsRef<Path>) -> anyhow::Result<Self> {
        let path = path.as_ref();
            ...
            "persistent ledger requires ordinary file path"
        );
        ...
        self.transaction(|tx| {
            if policy.enforcement == BudgetEnforcement::Hard {
                for a in account_attempts(tx, key_id, policy.unit)? {
                        ...
                        !holds_customer(&a) || a.reservation.bound_kind == BoundKind::Guaranteed,
                        "unbounded obligations prevent hard enforcement"
                    );
                }
            }
            tx.execute(
                "INSERT INTO ledger_accounts(key_id,unit,policy,cycle,used) VALUES(?1,?2,?3,1,'0')
    ...
    pub fn accounts(&self, key_id: u64) -> anyhow::Result<Vec<AccountView>> {
        self.transaction(|tx| {
            let rows = tx
                .prepare("SELECT policy FROM ledger_accounts WHERE key_id=?1 ORDER BY unit")?
                .query_map([key_id.to_string()], |r| r.get::<_, String>(0))?
                .collect::<Result<Vec<_>, _>>()?;
            rows.iter()
                .map(|p| account(tx, key_id, decode::<BudgetPolicy>(p)?.unit))
                .collect()
        })
    }
    ...
    pub fn settle(&self, input: SettlementInput) -> anyhow::Result<AttemptView> {
        ...
        self.transaction(|tx| {
            let mut view = attempt(tx, &input.attempt_id)?;
            if event_matches(tx, &input.settlement_id, "settlement", &input)? {
                return Ok(view);
            }
            if let Some(old) = &view.settlement {
                ensure!(
                    old.evidence != EvidenceStatus::Confirmed,
                    "attempt already finally settled"
                );
            }
            ensure!(
                ...
                input.committed || input.downstream_amount.is_none_or(|a| a == Amount::ZERO),
                "hidden attempt cannot debit customer"
            );
            update_record(
                tx,
                &mut view,
                AttemptInput {
                    event_id: input.settlement_id.clone(),
                    attempt_id: input.attempt_id.clone(),
                    cost: input.attempt_cost.clone(),
            ...
            if input.committed && input.evidence == EvidenceStatus::Confirmed {
                let charge = input
                    .downstream_amount
                    .context("confirmed charge missing")?;
                if view.enforcement == BudgetEnforcement::Hard {
                    ensure!(
                        charge <= view.reservation.upper_bound.context("hard bound missing")?,
                        "confirmed charge exceeds guaranteed bound"
                    );
                }
...
pub fn validate_policy(p: &BudgetPolicy) -> anyhow::Result<()> {
        ...
        p.max_in_flight > 0 && p.max_pending > 0,
        "concurrency and pending limits must be positive"
    );
    ensure!(
        p.unit != BillingUnit::KiroCredit || p.enforcement == BudgetEnforcement::Soft,
        "native Kiro credits require soft enforcement"
    );
    for values in [&p.allowed_models, &p.allowed_upstreams] {
        let mut unique = std::collections::HashSet::new();
        for v in values {
            nonblank(v)?;
            ensure!(unique.insert(v), "duplicate authorization entry");
        }
    }
    Ok(())
}
fn validate_reservation(i: &ReservationInput) -> anyhow::Result<()> {
    for v in [
        ...
        &i.public_model,
        &i.upstream_id,
        &i.snapshot.binding_id,
        &i.snapshot.upstream_model,
    ] {
        nonblank(v)?;
    }
    ensure!(
        (i.bound_kind == BoundKind::Guaranteed) == i.upper_bound.is_some(),
        "bound kind and amount disagree"
    );
    if i.unit == BillingUnit::KiroCredit {
        ensure!(
            i.snapshot.upstream_kind == UpstreamKind::Kiro,
            "only Kiro provides native credit billing"
        );
        ensure!(
            i.bound_kind == BoundKind::Unknown,
            "native credit has no guaranteed bound"
        );
    } else {
        let sale = i
            .snapshot
            .sell_prices
            .as_ref()
            .context("monetary sale prices missing")?;
        sale.validate()?;
        ensure!(sale.currency == i.unit, "sale currency mismatch");
    }
    if i.snapshot.upstream_kind == UpstreamKind::Kiro {
        ensure!(
            i.snapshot.cost_prices.is_none(),
            "Kiro cost uses native credits"
        );
    } else {
        i.snapshot
            .cost_prices
            .as_ref()
            .context("cost prices missing")?
            .validate()?;
    }
    if let Some(p) = &i.snapshot.sell_prices {
        p.validate()?;
    }
    Ok(())
}
fn validate_evidence(status: EvidenceStatus, amount: Option<Amount>) -> anyhow::Result<()> {
        ...
        (status == EvidenceStatus::Confirmed) == amount.is_some(),
        "confirmed evidence requires amount; pending cannot invent amount"
    );
    Ok(())
}
fn update_record(
    tx: &Transaction<'_>,
    view: &mut AttemptView,
    i: AttemptInput,
) -> anyhow::Result<()> {
    validate_evidence(i.cost.evidence, i.cost.amount)?;
    let unit = if view.reservation.snapshot.upstream_kind == UpstreamKind::Kiro {
        ...
        view.reservation
            ...
            .cost_prices
            .as_ref()
            .context("frozen cost prices missing")?
            .currency
    };
    ensure!(i.cost.unit == unit, "provider cost currency mismatch");
    ensure!(
        i.committed || i.outcome != AttemptOutcome::Succeeded,
        "success requires committed response"
    );
    ensure!(
        view.state != ReservationState::Released || !i.committed,
        "released attempt cannot commit"
    );
    if let Some(old) = &view.record {
        ensure!(!old.committed || i.committed, "commitment cannot reverse");
        if old.cost.evidence == EvidenceStatus::Confirmed {
            ensure!(old.cost == i.cost, "confirmed provider cost is immutable");
        }
        if old.committed {
            ensure!(old.outcome == i.outcome, "committed outcome is immutable");
        }
    }
    if i.committed {
        let committed: Option<String> = tx.query_row(
            "SELECT committed_attempt_id FROM ledger_requests WHERE request_id=?1",
            [&view.reservation.request_id],
            |r| r.get(0),
        )?;
        ensure!(
            committed.as_ref().is_none_or(|id| id == &i.attempt_id),
            "another attempt committed request"
        );
        tx.execute(
            "UPDATE ledger_requests SET committed_attempt_id=?1 WHERE request_id=?2",
            params![i.attempt_id, view.reservation.request_id],
        )?;
    }
    view.record = Some(i);
    Ok(())
...
fn account(tx: &Transaction<'_>, key: u64, unit: BillingUnit) -> anyhow::Result<AccountView> {
    let (policy, cycle, used): (String, u64, String) = tx
        .query_row(
            "SELECT policy,cycle,used FROM ledger_accounts WHERE key_id=?1 AND unit=?2",
            params![key.to_string(), unit_name(unit)],
            |r| Ok((r.get(0)?, r.get(1)?, r.get(2)?)),
        )
        .optional()?
        .context("billing account missing")?;
    let policy: BudgetPolicy = decode(&policy)?;
    let used: Amount = used.parse()?;
    let (mut reserved, mut in_flight, mut customer_pending, mut provider_only_pending) =
        (Amount::ZERO, 0_u32, 0_u32, 0_u32);
    for a in account_attempts(tx, key, unit)? {
        if holds_customer(&a) {
            ensure!(
                a.cycle == cycle,
                "unresolved customer obligation in old cycle"
            );
            reserved = reserved.checked_add(a.reservation.upper_bound.unwrap_or(Amount::ZERO))?;
        }
        if a.state == ReservationState::InFlight {
            in_flight = in_flight.checked_add(1).context("count overflow")?;
        }
        if a.state == ReservationState::Pending {
            customer_pending = customer_pending.checked_add(1).context("count overflow")?;
        } else if a.state != ReservationState::InFlight
            && a.record
                .as_ref()
                .is_none_or(|r| r.cost.evidence == EvidenceStatus::Pending)
        {
            provider_only_pending = provider_only_pending
                .checked_add(1)
    ...
    Ok(AccountView {
        ...
        pending: customer_pending
            .checked_add(provider_only_pending)
            .context("count overflow")?,
        customer_pending,
        provider_only_pending,
    })
}
fn account_attempts(
    tx: &Transaction<'_>,
    key: u64,
    unit: BillingUnit,
) -> anyhow::Result<Vec<AttemptView>> {
    let rows = tx
        .prepare("SELECT data FROM ledger_attempts WHERE key_id=?1 AND unit=?2 ORDER BY rowid")?
        .query_map(params![key.to_string(), unit_name(unit)], |r| {
            r.get::<_, String>(0)
        })?
        .collect::<Result<Vec<_>, _>>()?;
    rows.iter().map(|s| decode(s)).collect()
...
fn attempt_optional(tx: &Transaction<'_>, id: &str) -> anyhow::Result<Option<AttemptView>> {
    tx.query_row(
        "SELECT data FROM ledger_attempts WHERE attempt_id=?1",
        [id],
        |r| r.get::<_, String>(0),
    )
    .optional()?
...
fn attempt(tx: &Transaction<'_>, id: &str) -> anyhow::Result<AttemptView> {
    attempt_optional(tx, id)?.context("unknown attempt")
}
fn save_attempt(tx: &Transaction<'_>, a: &AttemptView) -> anyhow::Result<()> {
    tx.execute(
        "UPDATE ledger_attempts SET data=?1 WHERE attempt_id=?2",
        params![json(a)?, a.reservation.attempt_id],
    )?;
    Ok(())
}
...
fn set_used(tx: &Transaction<'_>, a: &AccountView, used: Amount) -> anyhow::Result<()> {
    tx.execute(
        "UPDATE ledger_accounts SET used=?1 WHERE key_id=?2 AND unit=?3",
        params![
            used.to_string(),
            a.key_id.to_string(),
            unit_name(a.policy.unit)
        ],
    )?;
    Ok(())
}
fn save_audit(tx: &Transaction<'_>, audit: AccountAuditView) -> anyhow::Result<()> {
    tx.execute(
        "INSERT INTO ledger_account_audit(operation_id,key_id,data) VALUES(?1,?2,?3)",
...
fn migration_complete(tx: &Transaction<'_>) -> anyhow::Result<bool> {
    let value: Option<String> = tx
        .query_row(
            "SELECT value FROM ledger_metadata WHERE name='legacy_migration_complete'",
            [],
            |r| r.get(0),
        )
        .optional()?;
    match value.as_deref() {
        None => Ok(false),
        Some("true") => Ok(true),
        _ => anyhow::bail!("invalid migration marker"),
    }
}
fn event_matches(
    tx: &Transaction<'_>,
    ...
    let old: Option<(String, String)> = tx
        .query_row(
            "SELECT kind,payload FROM ledger_events WHERE event_id=?1",
            [id],
    ...
    if let Some((old_kind, payload)) = old {
            ...
            old_kind == kind && payload == json(value)?,
            "conflicting idempotency event"
        );
        Ok(true)
    } else {
        Ok(false)
...
fn insert_event(
    ...
    tx.execute(
        "INSERT INTO ledger_events(event_id,kind,payload) VALUES(?1,?2,?3)",
        params![id, kind, json(value)?],
    )?;
    Ok(())
}
fn holds_customer(a: &AttemptView) -> bool {
    matches!(
        a.state,
        ReservationState::InFlight | ReservationState::Pending
    )
}
fn allowed(values: &[String], target: &str) -> bool {
    values.is_empty() || values.iter().any(|v| v == target)
}
fn nonblank(value: &str) -> anyhow::Result<()> {
    ensure!(
        !value.trim().is_empty(),
        "identifier/reason cannot be blank"
    );
    Ok(())
}
fn unit_name(unit: BillingUnit) -> &'static str {
    match unit {
        BillingUnit::KiroCredit => "kiro_credit",
        BillingUnit::Cny => "CNY",
        BillingUnit::Usd => "USD",
    }
}
fn json(value: &impl Serialize) -> anyhow::Result<String> {
    Ok(serde_json::to_string(value)?)
}
fn decode<T: DeserializeOwned>(value: &str) -> anyhow::Result<T> {
