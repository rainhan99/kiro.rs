...
/// Compaction 专用入口：上游 0.9.0 的摘要通道走这里。
///
/// 它同样经过 pipeline 配置，因此净化、预算与工具结果形状对摘要请求一视同仁——
/// 摘要是客户端（Codex 的 `/compact`）显式请求的操作，不是网关自行决定的降级。
...
fn convert_request_inner(
    ...
    // Compact 是纯摘要通道，不带工具；生成路径仍走 pipeline 的 preserve 语义。
