...
结论：**阶段 1（2026-09-17）与阶段 2（2026-09-18）已全部实现，阶段 3 仍未全部满足。** 阶段 1 的四项按「量具 → 账单 → 分类 → 补救」一条链交付，spec 见 `docs/superpowers/specs/2026-09-17-request-pipeline-phase1.md`。阶段 2 的 spec 见 `docs/superpowers/specs/2026-09-18-request-pipeline-phase2.md`，按「观测 → 聚合 → 准入 → 恢复」的顺序交付：先留下上游已经告诉我们、但一直被丢弃的东西，再从本来就会发生的流量里聚合，之后才让上限去拦请求、让分类明确的拒绝被修正一次。顺序本身是这一阶段的实质——反过来做就是把猜测升级成门禁。阶段 3 的推理算法没有因此自动补齐。
...
后续开发按用户给定顺序继续：阶段 3 的同模型增量记忆 / Map-Reduce / 按需工具目录分页。任何长度恢复都不能盲目原样重发、自动降级模型或启动线上阈值二分。
