      ...
      <form onSubmit={(event) => { event.preventDefault(); void submit() }} className="space-y-6">
        <fieldset disabled={!editable || busy} className="min-w-0 space-y-7 disabled:opacity-65">
          ...
          <SettingGroup title="执行与审计" description="各开关只修改草稿，统一通过下方保存按钮提交。">
            ...
            {select('unexpressible', '默认「历史转为可移植文本」只处理跨上游的历史：可读文本与公共搜索摘要保留，provider 私有签名和不透明数据会移除。portable-text 与「遇到不兼容内容就拒绝」都会拒绝当前输入（最后一条用户消息）的不兼容内容，不会为了跑通而降级；旧版「丢弃并记录」会有意丢弃这类当前内容和末尾 assistant prefill。转换告警和审计仅留在本平台，不会发送给 Kiro；与执行模式正交。')}
