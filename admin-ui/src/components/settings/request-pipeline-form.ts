...
export const numericFields = [
  ...
  { key: 'toolCatalog.budgetBytes', label: '工具声明字节预算', min: 1024, max: 104857600, unit: '字节' },
  { key: 'chunkedMap.chunkBytes', label: '分块大小', min: 1024, max: 10485760, unit: '字节' },
  { key: 'chunkedMap.maxChunks', label: '块数上限', min: 1, max: 64, unit: '块' },
...
export function validateDraft(draft: PipelineDraft): { config?: PipelineConfig; errors: Record<string, string> } {
  ...
  return { errors, config: {
    ...
    toolCatalog: { strategy: draft['toolCatalog.strategy'] as PipelineConfig['toolCatalog']['strategy'], budgetBytes: n('toolCatalog.budgetBytes') },
    chunkedMap: { strategy: draft['chunkedMap.strategy'] as PipelineConfig['chunkedMap']['strategy'], chunkBytes: n('chunkedMap.chunkBytes'), maxChunks: n('chunkedMap.maxChunks') },
