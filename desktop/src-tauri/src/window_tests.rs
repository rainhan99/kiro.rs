...
/// 「每次启动都要验证」开关。
///
/// 默认关：日常不碍事。打开后桌面端启动时清掉存下来的密钥，于是进门
/// 要输一遍密码——那才是真的门，有人走到你没锁屏的电脑前也进不去。
#[test]
fn the_forget_script_is_emitted_only_when_the_setting_is_on() {
    assert!(forget_key_script_for(true).is_some());
    assert!(forget_key_script_for(false).is_none());
}

/// 清的是登录密钥，不能连主题、折叠状态之类的一起清掉——那会让人
/// 每次开应用都觉得「我的设置怎么没了」。
#[test]
fn the_forget_script_removes_only_the_login_key() {
    let script = forget_key_script_for(true).unwrap();
    assert!(script.contains("removeItem"), "实际: {script}");
    assert!(script.contains("\"adminApiKey\""), "实际: {script}");
    assert!(
        !script.contains("clear()"),
        "不能用 localStorage.clear()——会把主题等无关设置一起清掉：{script}"
    );
}

/// 注入 setup token 与清掉登录密钥可能同时发生（首启动 + 开关开着）。
/// 两者都要注入，顺序无所谓，但不能互相覆盖。
#[test]
fn both_scripts_can_be_injected_together() {
    let combined = injection_script(Some("TOKEN123"), true).unwrap();
    assert!(combined.contains("kiroSetupToken"), "实际: {combined}");
    assert!(combined.contains("removeItem"), "实际: {combined}");
}

#[test]
fn nothing_is_injected_when_there_is_nothing_to_do() {
    assert!(injection_script(None, false).is_none());
}
